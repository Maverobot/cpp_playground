...
std::array<int, sizeof...(Is)> f_all_dispatch(Op&& op, Tuple&& t, std::index_sequence<Is...>) {
...
std::array<int, std::tuple_size<Tuple>::value> execute_all(Op&& op, const Tuple& t) {
  return f_all_dispatch(op, t, std::make_index_sequence<std::tuple_size<Tuple>::value>{});
}
}  // namespace simple_test
...
auto execute_all(Op&& op, const Tuple& t) {
  return detail::f_all_dispatch(op, t, std::make_index_sequence<std::tuple_size<Tuple>::value>{});
}
