 ...
 */
static bt_value* create_ctf_lttng_live_comp_params(const std::string& listening_url) {
  bt_value* params;
  ...
  CheckBtError(bt_value_array_append_string_element(url_array, listening_url.c_str()));
...
static bt_graph_add_component_status add_ctf_lttng_live_comp(
    ...
    const std::string& listening_url,
...
static bt_graph* create_graph(const char* const trace_dir, struct relay_data* const relay_data) {
  ...
  if (add_comp_status != BT_GRAPH_ADD_COMPONENT_STATUS_OK) {
    std::cout << "Failed to add component 'ctf_lttng_live'. Returned status: " << add_comp_status
              << "\n";
    goto error;
  }
