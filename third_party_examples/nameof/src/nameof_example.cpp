...
template <template <typename...> class TypeList, typename... Ts, typename... TsNew>
...
template <template <typename...> class TypeList, typename... T1s, typename... T2s>
...
struct type_in_list {
  static constexpr bool value = sizeof...(Ts) != 0 && ((std::is_same_v<T, Ts>) || ...);
};

template <template <typename...> class Tlist, class T, class... Trest>
struct unique_types<Tlist<T, Trest...>> {
  using type =
      std::conditional_t<type_in_list<T, Trest...>::value,
...
int main(int, char**) {
  ...
  std::cout << "unique1: " << nameof::nameof_type<unique_type_list<type1>::type>() << std::endl
            << std::endl;

  std::cout << "  type2: " << nameof::nameof_type<type2>() << std::endl;
  std::cout << "unique2: " << nameof::nameof_type<unique_type_list<type2>::type>() << std::endl
            << std::endl;

  std::cout << "  type3: " << nameof::nameof_type<type3>() << std::endl;
  std::cout << "unique3: " << nameof::nameof_type<unique_type_list<type3>::type>() << std::endl
            << std::endl;

  std::cout << "  type4: " << nameof::nameof_type<type4>() << std::endl;
  std::cout << "unique4: " << nameof::nameof_type<unique_type_list<type4>::type>() << std::endl;
}
