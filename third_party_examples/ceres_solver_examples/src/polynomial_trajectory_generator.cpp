...
struct QuinticTrajectoryGeneratorResidual {
  QuinticTrajectoryGeneratorResidual(double t, double x, double dx) : t_(t), x_(x), dx_(dx) {}
  template <typename T>
  bool operator()(const T* const a,
    ...
    residual[0] = x_ - get_polynomial_functor(a[0], b[0], c[0], d[0], e[0], f[0])(t_);

    // residual = dx - (5*a*x^4 + 4*b*x^3 + 3*c*x^2 + 2*d*x + e)
    residual[1] = dx_ - get_polynomial_functor_derivative(a[0], b[0], c[0], d[0], e[0], f[0])(t_);
    return true;
  }
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < kNumObservations; ++i) {
    problem.AddResidualBlock(
        new AutoDiffCostFunction<QuinticTrajectoryGeneratorResidual,
                                 QuinticTrajectoryGeneratorResidual::num_residuals, 1, 1, 1, 1, 1,
                                 1>(
            new QuinticTrajectoryGeneratorResidual(t[i], positions[i], velocities[i])),
        NULL, &a, &b, &c, &d, &e, &f);
  }
