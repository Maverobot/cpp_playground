...
class StateDependentDamper : public LeafSystem<T> {
 public:
  StateDependentDamper(const multibody::MultibodyPlant<T>& plant,
    ...
    this->DeclareVectorOutputPort(BasicVector<T>(num_v), &StateDependentDamper<T>::CalcTorque);
...
template <typename T>
KukaTorqueController<T>::KukaTorqueController(const multibody::MultibodyPlant<T>& plant,
                                              const VectorX<double>& stiffness,
  ...
  */
  ...
  input_port_index_estimated_state_ =
      builder.ExportInput(gravity_comp->get_input_port_estimated_state());
  ...
  input_port_index_desired_state_ = builder.ExportInput(spring->get_input_port_desired_state());

  // Exposes the commanded torque port.
  input_port_index_commanded_torque_ = builder.ExportInput(adder->get_input_port(0));
