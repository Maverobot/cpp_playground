...
int DoMain() {
  ...
  if (FLAGS_torque_control) {
    ...
    controller = builder.AddSystem<KukaTorqueController<double>>(plant, stiffness, damping_ratio);
  } else {
    ...
    controller = builder.AddSystem<InverseDynamicsController<double>>(
        plant, iiwa_kp, iiwa_ki, iiwa_kd, false /* without feedforward acceleration */);
  }

  // Create the command subscriber and status publisher.
  auto command_sub = builder.AddSystem(
      systems::lcm::LcmSubscriberSystem::Make<drake::lcmt_iiwa_command>("IIWA_COMMAND", lcm));
  ...
  auto desired_state_from_position = builder.AddSystem<StateInterpolatorWithDiscreteDerivative>(
      num_joints, kIiwaLcmStatusPeriod, true /* suppress_initial_transient */);
  desired_state_from_position->set_name("desired_state_from_position");
  auto status_pub = builder.AddSystem(systems::lcm::LcmPublisherSystem::Make<lcmt_iiwa_status>(
  ...
  builder.Connect(command_sub->get_output_port(), command_receiver->get_message_input_port());
  ...
  builder.Connect(plant.get_state_output_port(iiwa_instance), plant_state_demux->get_input_port(0));
  builder.Connect(plant_state_demux->get_output_port(0),
  ...
  builder.Connect(plant.get_generalized_contact_forces_output_port(iiwa_instance),
                  status_sender->get_torque_external_input_port());
  builder.Connect(status_sender->get_output_port(), status_pub->get_input_port());
