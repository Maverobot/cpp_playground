 ...
 */
template <typename T>
class KukaTorqueController : public systems::Diagram<T>,
 ...
 public:
  ...
  const systems::InputPort<T>& get_input_port_commanded_torque() const {
    return systems::Diagram<T>::get_input_port(input_port_index_commanded_torque_);
  }

  const systems::InputPort<T>& get_input_port_estimated_state() const override {
    return systems::Diagram<T>::get_input_port(input_port_index_estimated_state_);
  }
