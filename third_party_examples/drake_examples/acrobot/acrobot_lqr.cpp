...
DEFINE_string(model, "../acrobot/acrobot.sdf", "Path to acrobot.sdf file.");
...
DEFINE_double(simulation_time, 3.0, "Desired duration of the simulation in seconds.");

DEFINE_bool(time_stepping,
            true,
...
std::unique_ptr<systems::AffineSystem<double>> MakeBalancingLQRController(
  ...
  const RevoluteJoint<double>& shoulder = acrobot.GetJointByName<RevoluteJoint>("ShoulderJoint");
  const RevoluteJoint<double>& elbow = acrobot.GetJointByName<RevoluteJoint>("ElbowJoint");
  ...
  acrobot.get_applied_generalized_force_input_port().FixValue(context.get(),
...
int do_main() {
  ...
  RevoluteJoint<double>& shoulder = acrobot.GetMutableJointByName<RevoluteJoint>("ShoulderJoint");
  RevoluteJoint<double>& elbow = acrobot.GetMutableJointByName<RevoluteJoint>("ElbowJoint");

  // Drake's parser will default the name of the actuator to match the name of
  // the joint it actuates.
  const JointActuator<double>& actuator = acrobot.GetJointActuatorByName("ElbowJoint");
  ...
  auto controller = builder.AddSystem(MakeBalancingLQRController(model_path));
  ...
  for (int i = 0; i < 5; i++) {
    ...
    simulator.get_system().SetRandomContext(&simulator.get_mutable_context(), &generator);
